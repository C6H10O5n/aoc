...
namespace adventOfCode
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                int sum = 0;
                tbInputLines.Lines = data.Select(s =>
                {
                    var w = s;

                    var d1i = w.ToList().FindIndex(Char.IsAsciiDigit);
                    foreach (var k in nums.Keys) nums[k][1] = w.IndexOf(k);
                    var fk = nums.Where(x => x.Value[1] == nums.Values.Select(z => z[1] <= -1 ? 9999 : z[1]).Min()).FirstOrDefault();
                    if (fk.Value != null && fk.Value[1] > -1 && fk.Value[1] < d1i) w = w[..fk.Value[1]] + fk.Value[0].ToString() + w[(fk.Value[1] + fk.Key.Length)..]; //w.Replace(fk.Key, fk.Value[0].ToString());


                    foreach (var k in nums.Keys) nums[k][1] = w.LastIndexOf(k);
                    var lk = nums.Where(x => x.Value[1] == nums.Values.Select(z => z[1]).Max()).FirstOrDefault();
                    if (lk.Value != null && lk.Value[1] > -1) w = w[..lk.Value[1]] + lk.Value[0].ToString() + w[(lk.Value[1] + lk.Key.Length)..]; //w.Replace(lk.Key, lk.Value[0].ToString());


                    var x = Regex.Replace(w, "[^0-9.]", "");
                    var y =
                          x.Length == 1 ? x + x
                        : x.Length > 2 ? new string(new char[] { x.First(), x.Last() })
                        : x.Length <= 0 ? "00"
                        : x;
                    sum += int.Parse(y);
                    //return $"{y} |{x} | {w} | {s}";
                ...
                lbAnswer.Text = $"Answer: count={data.Count()} sum[first and last digit]={sum}";
            };
            ...
            bnRun2.Click += (s, e) =>
            {
                ...
                tbInputLines.Lines = data.Select(s =>
                {
                    ...
                    for (int i = 0; d1==0 && i < sp.Length; i++)
                    {
                        ...
                        foreach (var k in nums.Keys) { if (sp[i..].StartsWith(k)) { d1 = nums[k][0]; break; } }
                    }
                    ...
                    for (int i = sp.Length - 1; d2==0 && i >= 0; i--)
                    {
                        ...
                        foreach (var k in nums.Keys) { if (sp[i..].StartsWith(k)) { d2 = nums[k][0]; break; } }
                    }
            ...
            //init imput text
            tbInputLines.Lines = data;
        }
        Dictionary<string, int[]> nums = "one|1,two|2,three|3,four|4,five|5,six|6,seven|7,eight|8,nine|9".Split(",").ToDictionary(x => x.Split("|")[0], x => new int[] { int.Parse(x.Split("|")[1]), -1 });
        string[] data0 =
            """
            9cpjmdgf
            1eightwom
